# AWS IaC with CloudFormation Template to create a VPC, EC2, ASG and ALB

This CloudFormation template deploys a robust network architecture on AWS following the best practice. It uses the power of Infrastructure as Code to automatically create a Virtual Private Cloud (VPC), subnets, NAT gateways, EC2 instances, Auto Scaling Group (ASG) and an Application Load Balancer (ALB). The template also sets up the necessary security groups for the EC2 instances and the ALB. Here is the architecture diagram:
